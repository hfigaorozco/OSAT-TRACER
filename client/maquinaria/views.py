...
def admin_maquinaria(request):
    ...
    tipos_map = {str(t.get('clave', '')): t for t in tipos_maquina_bd}
    estados_map = {str(e.get('clave', '')): e for e in estados_maquina_bd}
    ...
    # Filtros reales (tipo/estado/línea) vía GET
...
def admin_maquinaria_editar(request, pk):
    if request.method == 'POST':
